...
namespace MsgServer.SocketServer.SocketPool
{
    public class SocketSenderQueue:IDisposable
    {
        ...
        private void sendAction(object obj)
        {
            ...
            if (connSocket != null)
            {
                try
                {
                    connSocket.Send(arg.MessageBuffer);
        ...
        private void sendTickAction(object obj)
        {
            ...
            foreach(var s in this.socketPool.GetAll())
            {
                ...
                try
                {
                    if (s.Value != null)
                    {
                        s.Value.Send(arg.MessageBuffer);
                        err = SocketError.Success;
                    }
                    else
                    {
    ...
    public class SocketSenderArgs
    {
        ...
        public byte[] MessageBuffer {
            get
            {
                if (this.MessageFormatting != null)
                {
                    return Encoding.Default.GetBytes(this.MessageFormatting(this.Message));
                }
                else
                {
