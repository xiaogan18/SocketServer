...
namespace MsgServer.SocketServer.Filter
{
    public class SocketHandshake:ISocketFilter
    {
        ...
        private string decrypt(byte[] buffer)
        {
            try
            {
                //var data = Convert.FromBase64String(Encoding.UTF8.GetString(buffer));
                //int offset = 0;
                //int max = 128;
                //string result = "";
                ...
                //            this.rsa.Decrypt(data.Skip(offset).Take(max).ToArray(), false));
                //    }
                //    else
                //    {
                //        result += Encoding.UTF8.GetString(this.rsa.Decrypt(data.Skip(offset).ToArray(), false));
                //    }
                //    offset += max;
                //}
                //return result;
